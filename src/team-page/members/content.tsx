...
import { AdditionalPlayerInfo } from "./AdditionalPlayerInfo";
...
const injectAvgFaceitElo = (playerInfos: PlayerInfo[]): void => {
    ...
    teamHeader.parentNode.appendChild(
        <h2 title="Durchschnittliche FACEIT Elo">
            {`FACEIT Elo: ${Math.round(avgElo)}`}
        </h2>
    );
}
    ...
    playerInfos.forEach(({ playerInfo, sourceElement }) => {
        if (playerInfo !== null) {
            sourceElement.appendChild(
                AdditionalPlayerInfo({ playerInfo }),
            );
        }
    });
};
    ...
    chrome.runtime.sendMessage(
        ...
        (playerInfos: PlayerInfo[]) => {
            const playerInfoWrappers = playerInfos.map((playerInfo) => ({
                playerInfo,
                sourceElement: memberCardsBySteamId2[playerInfo.steamId.get2()],
            }));
